...
class Dungeon {
 public:
  ...
  enum Direction { North, South, West, East, DirectionCount };
 ...
 public:
  ...
  void generate(int maxFeatures) {
    ...
    if (!makeRoom(_width / 2, _height / 2,
  ...
  void print() {
    for (int y = 0; y < _height; ++y) {
      for (int x = 0; x < _width; ++x) std::cout << getTile(x, y);
 ...
 private:
  char getTile(int x, int y) const {
    if (x < 0 || y < 0 || x >= _width || y >= _height) return Unused;

    return _tiles[x + y * _width];
  }

  void setTile(int x, int y, char tile) { _tiles[x + y * _width] = tile; }

  bool createFeature() {
    for (int i = 0; i < 1000; ++i) {
      if (_exits.empty()) break;
  ...
  bool makeCorridor(int x, int y, Direction dir) {
    ...
    if (placeRect(corridor, Corridor)) {
      ...
      if (dir != North && corridor.width != 1)  // south side
        _exits.emplace_back(
            Rect{corridor.x, corridor.y + corridor.height, corridor.width, 1});
      ...
      if (dir != West && corridor.height != 1)  // east side
        _exits.emplace_back(
            Rect{corridor.x + corridor.width, corridor.y, 1, corridor.height});
